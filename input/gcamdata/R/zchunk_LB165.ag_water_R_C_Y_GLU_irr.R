...
module_aglu_LB165.ag_water_R_C_Y_GLU_irr <- function(command, ...) {
  if(command == driver.DECLARE_INPUTS) {
    ...
    L165.ag_Water_ctry_crop_GLU %>%
     ...
     # ORIGINAL GCAM DATA SYSTEM
      group_by(GCAM_region_ID, GCAM_commodity, GLU) %>%

     # BY 1/3/22: GROUPING TO HAVE WATER FOOTPRINT BY COUNTRY (comment out when running normally)
   ...
   # BY 1/3/22: GROUPING TO HAVE WATER FOOTPRINT BY COUNTRY (comment out when running normally)
   #L165.BlueIrr_m3kg_R_C_GLU <- select(L165.ag_Water_R_C_GLU, iso, GCAM_commodity, BlueIrr_m3kg)
   #L165.TotIrr_m3kg_R_C_GLU <- select(L165.ag_Water_R_C_GLU, iso, GCAM_commodity, TotIrr_m3kg)
   #L165.GreenRfd_m3kg_R_C_GLU <- select(L165.ag_Water_R_C_GLU, iso, GCAM_commodity, GreenRfd_m3kg)
